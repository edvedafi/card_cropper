...
def detect_card(image_path, output_path, border_size=5):
    ...
    try:
        ...
        try:
            ...
            if len(results.xyxy[0]) > 0:
                ...
                if w < 10 or h < 10 or w > width * 1.5 or h > height * 1.5:
                    raise ValueError("Invalid dimensions detected")
                
                # Add border to dimensions
                output_width = w + (2 * border_size)
                output_height = h + (2 * border_size)
        ...
        # Fallback to adaptive thresholding if YOLO fails
        ...
        return False, f"Error in ML detection: {str(e)}" 